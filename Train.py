...
parser.add_argument('--learning_rate', default= 1e-4, type=float, help='optimizer learning rate')
parser.add_argument('--log_dir', default= r"logs_one_channel/", type=str, help='log folder were train model will be saved')
parser.add_argument('--resume_training_from', default= r"sam_vit_h_4b8939.pth", type=str, help='path to model to resume training from')
...
sam = sam_model_registry[model_type](checkpoint=  args.resume_training_from) # Build model all parts, load trained weight, set model to eval
...
for itr in range(InitStep,1000000):
    ...
    with torch.enable_grad():
       ...
       # predictor.model.mask_decoder.zero_grad()
        ...
        loss = Get_Loss_one_channel(high_res_masks,PointMasks,Imgs, PointsXY, NumPointMasks,iou_predictions)
     #   print("loss",loss)
     ...
     # #   loss=low_res_masks.mean().abs()#.requires_grad(True)
        ...
        if itr % 200 == 0 and itr > 0:  # Save model weight and other paramters in temp file once every 1000 steps
            ...
            torch.save(predictor.model.state_dict(), args.log_dir + "/Defult.torch")
            torch.save(predictor.model.state_dict(), args.log_dir + "/DefultBack.torch")
            print("model saved")
        ...
        if itr % 1000 == 0 and itr > 0:  # Save model weight once every 30k steps permenant (not temp)
            print("Saving Model to file in " + args.log_dir + "/" + str(itr) + ".torch")
            torch.save(predictor.model.state_dict(), args.log_dir + "/" + str(itr) + ".torch")
            print("model saved")
        #==========================================================================================================
